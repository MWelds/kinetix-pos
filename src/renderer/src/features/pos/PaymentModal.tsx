...
interface ReceiptConfig { template: string; showLogo: boolean; footer: string; logoBase64: string }
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  <table>
    ...
    ${snap.discountAmount > 0 ? `<tr><td>Discount</td><td style="text-align:right">-${fmtR(snap.discountAmount)}</td></tr>` : ''}
    ${snap.taxAmount > 0 ? `<tr><td>Tax</td><td style="text-align:right">${fmtR(snap.taxAmount)}</td></tr>` : ''}
  ...
  ${snap.notes ? `<div class="divider"></div><p>Note: ${esc(snap.notes)}</p>` : ''}
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  <div class="header">
    ...
    <h1>${esc(storeName)}</h1>
    <p>Order #${snap.orderNumber} &bull; ${new Date().toLocaleString()}</p>
  </div>
  <div class="body">
    ...
    <div class="total-section">
      ...
      ${snap.discountAmount > 0 ? `<div class="total-row" style="color:#10b981"><span>Discount</span><span>-${fmtR(snap.discountAmount)}</span></div>` : ''}
      ${snap.taxAmount > 0 ? `<div class="total-row"><span>Tax</span><span>${fmtR(snap.taxAmount)}</span></div>` : ''}
    ...
    <div class="payment-section">
      ...
      ${snap.change > 0.01 ? `<div class="payment-row" style="color:#10b981"><span>Change (${snap.changeCurrency})</span><span>${CURRENCIES[snap.changeCurrency].symbol}${snap.change.toFixed(2)}</span></div>` : ''}
    </div>
    ${snap.notes ? `<p style="margin-top:12px;font-size:12px;color:#64748b">Note: ${esc(snap.notes)}</p>` : ''}
  </div>
...
export function PaymentModal({ isOpen, onClose, onComplete }: PaymentModalProps) {
  ...
  const [receiptConfig, setReceiptConfig] = useState({ template: 'classic', showLogo: true, footer: 'Thank you for your business!', logoBase64: '' })
...
<div style="display:flex;justify-content:flex-end;margin-top:16px">
  <table class="totals" style="width:220px">
    ...
    ${snap.discountAmount > 0 ? `<tr><td>Discount</td><td style="text-align:right;color:#10b981">-${fmtAmt(snap.discountAmount)}</td></tr>` : ''}
